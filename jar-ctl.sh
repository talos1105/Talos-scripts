...
render_status_badge() {
  ...
  if [[ -n "$pid" ]]; then
    printf "${C_GREEN}● RUNNING${C_RESET}  PID %-7s" "$pid"
  else
...
render_status_board() {
  ...
  for pid_file in "$RUN_DIR"/*.pid; do
    ...
    printf "  %-22s  %b\n" "$name" "$(render_status_badge "$name")"
    found=1
  done

  [[ "$found" -eq 0 ]] && printf "  ${C_DIM}(no services found in run dir)${C_RESET}\n"
  divider
  echo ""
}
...
prompt_spring_profile() {
  ...
  divider
  echo ""
  printf "  ${C_GREEN}[1]${C_RESET}  prod   -- Production\n"
  printf "  ${C_GREEN}[2]${C_RESET}  uat    -- User Acceptance Testing\n"
  printf "  ${C_GREEN}[3]${C_RESET}  sit    -- System Integration Testing\n"
  printf "  ${C_GREEN}[4]${C_RESET}  other  -- Enter custom profile\n"
  echo ""
  divider
  ...
  while true; do
    ...
    read -r choice
    case "$choice" in
      1) SPRING_PROFILE="prod"; break ;;
      2) SPRING_PROFILE="uat";  break ;;
      3) SPRING_PROFILE="sit";  break ;;
      4)
        printf "  Enter custom profile name: "
        read -r SPRING_PROFILE
        [[ -n "$SPRING_PROFILE" ]] && break || log_warn "Profile name cannot be empty."
        ;;
      *) log_warn "Invalid selection. Enter 1-4." ;;
    esac
  done
...
action_start() {
  ...
  while (( wait < 10 )); do
    ...
    if ! kill -0 "$app_pid" 2>/dev/null; then
      sleep 0.5  # let tail flush remaining output
  ...
  # Confirmed running — keep tailing, Ctrl+C to detach
  trap 'echo ""; log "Detached. Service still running (PID $app_pid)."; kill "$tail_pid" 2>/dev/null; exit 0' INT
  wait "$tail_pid"
}
